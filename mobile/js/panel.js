 ...
 */
 ...
 function initSynoPanel(_object_id) {
    jeedom.object.all({
        ...
        success: function (objects) {
            ...
            for (var i in objects) {
                if (objects[i].isVisible == 1) {
                    ...
                    li += '<li></span><a href="#" class="link" data-page="panel" data-plugin="synoaudio" data-title="' + icon.replace(/\"/g, "\'") + ' ' + objects[i].name + '" data-option="' + objects[i].id + '"><span>' + icon + '</span> ' + objects[i].name + '</a></li>';
                }
            }
            li += '</ul>';
            panel(li);
...
function displaySyno(_object_id) {
    $.showLoading();
    $.ajax({
        type: 'POST',
