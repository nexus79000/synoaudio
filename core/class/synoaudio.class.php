										...
										if ($obj->data->playlist_total != 0){
											if ($cover != $cmd_track_image->execCmd()) {
												...
												if (!stristr(synoaudio::getCurlPage($cover),'not found')){
													...
													file_put_contents(dirname(__FILE__) . '/../../../../plugins/synoaudio/doc/images/syno_cover_' . $eqLogic->getId() . '.jpg', synoaudio::getCurlPage($cover));
													$changed = true;
												} else {
													log::add('synoaudio', 'debug', ' Pas de Cover ');
													if (file_exists(dirname(__FILE__) . '/../../../../plugins/synoaudio/doc/images/syno_cover_' . $eqLogic->getId() . '.jpg')) {
														unlink(dirname(__FILE__) . '/../../../../plugins/synoaudio/doc/images/syno_cover_' . $eqLogic->getId() . '.jpg');
													}
												}
											}
										}else{
...
*/		log::add('synoaudio', 'debug', ' GetPlayList - Fin' );
	...
	public function toHtml($_version = 'dashboard') { //Fini
		...
		$replace = array(
			...
			'#uid#' => 'synoaudio' . $this->getId() . self::UIDDELIMITER . mt_rand() . self::UIDDELIMITER,
			'#color#' => '#ffffff',
			'#border#' => 'none',
			'#border-radius#' => '4px',
			'#style#' => '',
		...
		if (is_object($cmd_track_image)) {
			$img = dirname(__FILE__) . '/../../../../plugins/synoaudio/doc/images/syno_cover_' . $this->getId() . '.jpg';
			if (file_exists($img) && filesize($img) > 100) {
				$replace['#thumbnail#'] = 'plugins/synoaudio/doc/images/syno_cover_' . $this->getId() . '.jpg?time=' .time();
			} else {
				$replace['#thumbnail#'] = 'plugins/synoaudio/doc/images/syno_cover_default.png?time=' .time();
			}
		}
		...
		return template_replace($replace, getTemplate('core', $_version, 'synoaudio', 'synoaudio'));
	}
