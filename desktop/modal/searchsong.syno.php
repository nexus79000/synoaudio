					...
					if ( isset($_GET['keyword']) && $_GET['keyword']!= '' ) {
						...
						if ($result->artistTotal != '0'){
							foreach ( $result->artists as $artist) {
								echo '<tr>';
								echo '<td>';
								echo '<a class="bt_playArtist btn btn-xs btn-success" data-syno_id="' . init('id') . '" data-name="' . $artist->name . '"><i class="fas fa-play"></i></a>';
								echo '</td>';
								echo '<td>';
								echo $artist->name;
								echo '</td>';
					...
					if ( isset($_GET['keyword']) && $_GET['keyword']!= '' ) {
						...
						if ($result->albumTotal != '0'){
							...
							foreach ( $result->albums as $album) {
								echo '<tr>';
								echo '<td>';
								echo '<a class="bt_playAlbum btn btn-xs btn-success" data-syno_id="' . init('id') . '" data-name="' . $album->name . '" data-artist="' . $album->album_artist . '"data-syno_id='. $i .'"><i class="fas fa-play"></i></a>';
								echo '<a class="bt_AffAlbum btn btn-xs btn-success"  data-alb_id="'. $i .'"><i class="fas fa-arrow-down"></i></a>';
								echo '</td>';
								echo '<td>';
								echo $album->name;
								echo '</td>';
...
?>
	<div id="chanson_album<?php echo $i;?>" class="listsong" style="overflow:auto; height: 50%; width: 98%; position: absolute; top: 50%; display:none;">
		<table class="table table-condensed">
			<thead>
				<tr>
				<th style="width : 60px;"><a class="btn btn-xs btn-success" id="bt_playSong" data-syno_id="<?php echo init('id');?>"><i class="fas fa-play"></i></a></th>
				<th>{{Chansons}}</th>
